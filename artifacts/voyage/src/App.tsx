...
function AppRoutes() {
  const { language, setLanguage } = usePlanContext();
  const { user, loading: authLoading } = useAuth();

  // Demo accounts: auto-set language so they skip the LanguagePicker
  React.useEffect(() => {
    if (user?.isDemoAccount && !language) {
      setLanguage('ru');
    }
  }, [user?.isDemoAccount, language, setLanguage]);
  ...
  // For demo accounts: show spinner briefly while language auto-sets
  if (!language && user.isDemoAccount) return <AuthLoadingScreen />;
  ...
  return (
    <Switch>
      {/* Demo accounts land on /plan (trip setup) instead of the marketing Home */}
      <Route path="/" component={user.isDemoAccount ? Plan : Home} />
