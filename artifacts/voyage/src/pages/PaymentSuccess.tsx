...
export default function PaymentSuccess() {
  ...
  useEffect(() => {
    ...
    const finish = async (premium: boolean) => {
      ...
      setVerifying(false);
      if (premium) {
        try { await refreshPremiumStatus(); } catch { /* ignore */ }
        try { localStorage.removeItem('voyage_pending_payment_id'); } catch { /* ignore */ }
      }
    };

    // ── 1. Try direct payment-ID verification first (fast & reliable) ─────────
    const checkByPaymentId = async (): Promise<boolean> => {
      let paymentId: string | null = null;
      try { paymentId = localStorage.getItem('voyage_pending_payment_id'); } catch { /* ignore */ }
      if (!paymentId) return false;

      try {
        const res = await fetch(`/api/yookassa/payment/${paymentId}`, { credentials: 'include' });
        if (!res.ok) return false;
        const data = await res.json() as PaymentCheckResult;

        if (data.isCanceled) {
          if (!cancelled) setLocation('/payment-failed');
          return true; // handled
        }
        if (data.isSucceeded || data.isPremium) {
          await finish(true);
          return true;
        }
        // Payment still pending — fall through to polling
        return false;
      } catch {
        return false;
      }
    ...
    // ── 2. Poll account premium status (covers webhook-activated premium) ─────
    ...
    const run = async () => {
      const handled = await checkByPaymentId();
      if (!handled) {
        // No specific payment ID or payment still pending — fall back to polling
