...
import { usePlanContext, STORAGE, type SavedTripParams } from '@/lib/plan-context';
...
export default function Paywall() {
  ...
  const handlePay = async () => {
    ...
    // ── Save full trip state so it can be restored after YooKassa redirect ──
    const sessionId = crypto.randomUUID();
    const tripParams: SavedTripParams = {
    ...
    try {
      localStorage.setItem(STORAGE.tripParams, JSON.stringify(tripParams));
      if (ctx.result) localStorage.setItem(STORAGE.generatedRoute, JSON.stringify(ctx.result));
    ...
    try {
      ...
      // Store payment ID as fallback (in case server session is lost after restart)
      if (data.paymentId) {
        try { localStorage.setItem(STORAGE.paymentId, data.paymentId); } catch { /* ignore */ }
      }
