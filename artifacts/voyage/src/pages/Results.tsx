...
export default function Results() {
  ...
  // Confirmed by user in the redirect modal — always open in a NEW tab, never replace Voyage
  const handleBookingConfirm = () => {
    const url = bookingModal?.url;
    setBookingModal(null);
    if (!url) return;

    // Primary: window.open in new tab (works in all modern browsers)
    const opened = window.open(url, '_blank', 'noopener,noreferrer');

    // Fallback for iOS/popup-blocked: temporary anchor click — still targets _blank
    // Never use window.location.href which destroys the current Voyage tab
